...
import {StaticFeatureFlags}       from "../../../../../configs/static-feature-flags"
import {portAllocator}            from "../../../../../utilities/network-utils/port-allocator"
...
export async function executePrismaRelatedProcesses() {
	...
	if (StaticFeatureFlags.shouldRunPrismaStudio) {
		...
		exec(
			`npx prisma studio -p ${ApplicationConfiguration.prismaAdminPort} --browser none`,
			(error, stdout, stderr) => {
				logger.verbose('-'.repeat(54))
