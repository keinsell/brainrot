...
export async function executePrismaRelatedProcesses() {
	...
	if (StaticFeatureFlags.shouldRunPrismaStudio) {
		...
		if (freePort.wasReplaced) {
			logger.warn(`Prisma Admin port availability check failed and ::${__appConfig.PRISMA_ADMIN_PORT} is not available, found a new shiny ::${freePort.port} instead. If you believe this is a mistake, please check your environment variables and processes that are running on your machine.`)
		} else {
			logger.log(`Prisma Admin port availability check succeeded and requested ::${__appConfig.PRISMA_ADMIN_PORT} is available`)
		}

		__appConfig.PRISMA_ADMIN_PORT = freePort.port

		exec(`npx prisma studio -p ${__appConfig.PRISMA_ADMIN_PORT} --browser none`, (error, stdout, stderr) => {
			logger.verbose('-'.repeat(54))
