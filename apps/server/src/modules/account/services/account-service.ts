...
export class AccountService
	 ...
	 implements AccountSelfService
	 {
		  ...
		  async register( registerAccount : RegisterAccountCommand ) : Promise<Account>
				{
					 const span = new OpentelemetryTracer().startSpan(
						  'com.methylphenidate.account_service.register',
						  {
					 ...
					 const password = await Password.fromPlain(
						  ...
						  this.hashing.use( KdfAlgorithm.Argon2id ),
					 )
					 ...
					 const createAccountQuery = await this.repository.create( identity )
					 
					 const isCreateAccountQueryErr = createAccountQuery.isErr()
					 
					 if ( isCreateAccountQueryErr )
						  {
								span.end()
								span.recordException( createAccountQuery.error )
								throw createAccountQuery.error
						  }
					 
					 identity = createAccountQuery.value
