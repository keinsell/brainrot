		  ...
		  public async sendVerificationEmail( accountId : string ) : Promise<void>
				{
					 ...
					 const isErrorInsteadAccount = account.isErr()
					 
					 if ( isErrorInsteadAccount )
						  {
								throw account.error
						  }
					 
					 const validAccount = account.value
					 ...
					 const emailVerificationSentEvent = new AccountVerificationEmailSent( validAccount )
