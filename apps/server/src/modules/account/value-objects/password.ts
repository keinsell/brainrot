...
}                from "../../../common/libraries/pwnproc/report/password-security-report.js"
import {
	PhcString
}                from "../../../common/libraries/unihash/types/phc-string.js"
import {
	UnihashAlgorithm
}                from "../../../common/libraries/unihash/unihash-algorithm.js"
...
export class Password
	...
	static async fromPlain(plain: string,
		hashingService: UnihashAlgorithm
	): Promise<Password> {
		const hash = await hashingService.hash(plain)
		return new Password({
			hash: PhcString.deserialize(hash), plain,
		})
	}
	
	
	public async compare(plain: string,
		hashingService: UnihashAlgorithm
	): Promise<boolean> {
	...
	public async generateReport(passwordStrengthEstimator: Pwnproc): Promise<PasswordSecurityReport> {
		if (!this.plain) {
			throw new Error(
				"Cannot generate report for a password that was not created from plain text.")
		}
