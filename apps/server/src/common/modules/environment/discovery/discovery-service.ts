...
import {
	DiscoveredClass, DiscoveredClassWithMeta, DiscoveredMethodWithMeta, Filter, MetaKey,
...
export const withMetaAtKey: (key: MetaKey) => Filter<DiscoveredClass> = (key) => (component) => {
	...
	const metaTargets: Function[] = [
		R.path(['instance', 'constructor'], component) as any, // eslint-disable-next-line @typescript-eslint/ban-types
...
export class DiscoveryService {
	 ...
	 */
	async methodsAndControllerMethodsWithMetaAtKey<T>(metaKey: MetaKey, metaFilter: Filter<T> = () => true): Promise<DiscoveredMethodWithMeta<T>[]> {
		const controllersWithMeta = (
			await this.controllersWithMetaAtKey<T>(metaKey)
		).filter((x) => metaFilter(x.meta));

		const methodsFromDecoratedControllers = R.chain(controller => this.classMethodsWithMetaAtKey<T>(controller.discoveredClass, PATH_METADATA), controllersWithMeta);

		const decoratedMethods = (
			await this.controllerMethodsWithMetaAtKey<T>(metaKey)
		).filter(x => metaFilter(x.meta));

		return R.uniqBy(x => x.discoveredMethod.handler, [...methodsFromDecoratedControllers, ...decoratedMethods]);
	}
	...
	async controllers(filter: Filter<DiscoveredClass>): Promise<DiscoveredClass[]> {
		...
		return (
			await this.discoveredControllers
		).filter((x) => filter(x));
	}
	...
	classMethodsWithMetaAtKey<T>(component: DiscoveredClass, metaKey: MetaKey): DiscoveredMethodWithMeta<T>[] {
		...
		.scanFromPrototype(instance, prototype, (name) => this.extractMethodMetaAtKey<T>(metaKey, component, prototype, name))
	 ...
	 */
	async providerMethodsWithMetaAtKey<T>(metaKey: MetaKey, providerFilter: Filter<DiscoveredClass> = () => true): Promise<DiscoveredMethodWithMeta<T>[]> {
		const providers = await this.providers(providerFilter);

		return R.chain((provider) => this.classMethodsWithMetaAtKey<T>(provider, metaKey), providers);
	}
	 ...
	 */
	async controllerMethodsWithMetaAtKey<T>(metaKey: MetaKey, controllerFilter: Filter<DiscoveredClass> = () => true): Promise<DiscoveredMethodWithMeta<T>[]> {
	...
	private async toDiscoveredClass(nestModule: Module, wrapper: InstanceWrapper<any>): Promise<DiscoveredClass> {
		const instanceHost = wrapper.getInstanceByContextId(STATIC_CONTEXT, wrapper && wrapper.id ?
			wrapper.id :
			undefined);
	...
	private extractMethodMetaAtKey<T>(metaKey: MetaKey, discoveredClass: DiscoveredClass, prototype: any, methodName: string): DiscoveredMethodWithMeta<T> {
	...
	private async discover(component: 'providers' | 'controllers') {
		...
		return Promise.all(R.chain(([key, nestModule]) => {
