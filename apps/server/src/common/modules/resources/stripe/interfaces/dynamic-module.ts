...
export function createConfigurableDynamicRootModule<T, U>(moduleConfigToken: InjectionToken, moduleProperties: Partial<Pick<ModuleMetadata, 'imports' | 'exports' | 'providers' | 'controllers'>> = {
	imports:   [],
	exports:   [],
	providers: [],
}) {
	abstract class DynamicRootModule {
		...
		static async externallyConfigured(moduleCtor: Type<T>, wait: number): Promise<DynamicModule> {
			const timeout$ = interval(wait).pipe(first(), map(() => {
